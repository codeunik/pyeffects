...
def bezier_point(p, t):
    ...
    deg = len(p) - 1
    if deg == 3:
        return p[0] + t * (3 * (p[1] - p[0]) + t *
                           (3 * (p[0] + p[2]) - 6 * p[1] + t *
                            (-p[0] + 3 * (p[1] - p[2]) + p[3])))
    elif deg == 2:
        return p[0] + t * (2 * (p[1] - p[0]) + t * (p[0] - 2 * p[1] + p[2]))
    elif deg == 1:
...
def bezier2polynomial(p, numpy_ordering=True, return_poly1d=False):
    ...
    if len(p) == 4:
        ...
        coeffs = [
            fac(n) // fac(n - j) * sum(
                (-1)**(i + j) * p[i] / (fac(i) * fac(j - i))
                for i in range(j + 1)) for j in range(n + 1)
        ]
        coeffs.reverse()
...
def bezier_real_minmax(p):
    ...
    if len(p) == 4:  # cubic case
        ...
        if denom != 0:
            delta = a[1]**2 - (a[0] + a[1]) * a[2] + a[2]**2 + (a[0] -
                                                                a[1]) * a[3]
...
def bezier_bounding_box(bez):
    ...
    try:
        bla = bez.large_arc
        return bez.bounding_box()  # added to support Arc objects
    except:
        pass
    # end arc support block ##########################
    ...
    dx = x.deriv()
    dy = y.deriv()
    x_extremizers = [0, 1] + polyroots(
        dx, realroots=True, condition=lambda r: 0 < r < 1)
    y_extremizers = [0, 1] + polyroots(
...
def bezier_intersections(bez1, bez2, longer_length, tol=1e-8, tol_deC=1e-8):
    ...
    while pair_list and k < maxits:
        ...
        for pair in pair_list:
            bounding_box1 = bezier_bounding_box(pair.bez1)
            bounding_box2 = bezier_bounding_box(pair.bez2)
            if boxes_intersect(bounding_box1, bounding_box2):
                if box_area(*bounding_box1) < tol_deC and box_area(
                        *bounding_box2) < tol_deC:
