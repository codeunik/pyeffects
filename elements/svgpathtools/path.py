...
from math import acos, asin, ceil, cos, degrees, log, pi, radians, sin, sqrt
...
def is_bezier_segment(seg):
    return (isinstance(seg, Line) or isinstance(seg, QuadraticBezier)
            or isinstance(seg, CubicBezier))
...
def bounding_box2path(xmin, xmax, ymin, ymax):
...
def transform(curve, tf):
    ...
    if isinstance(curve, Path):
        ...
        return bpoints2bezier(
            [to_complex(tf.dot(to_point(p))) for p in curve.bpoints()])
    elif isinstance(curve, Arc):
...
def segment_length(curve,
    ...
    if (length2 - length > error) or (depth < min_depth):
        ...
        return (segment_length(curve, start, mid, start_point, mid_point,
                ...
                segment_length(curve, mid, end, mid_point, end_point, error,
                               min_depth, depth))
    # This is accurate enough.
    return length2


def inv_arclength(curve,
                  s,
                  s_tol=ILENGTH_S_TOL,
                  maxits=ILENGTH_MAXITS,
                  error=ILENGTH_ERROR,
                  min_depth=ILENGTH_MIN_DEPTH):
    ...
    elif (isinstance(curve, QuadraticBezier) or isinstance(curve, CubicBezier)
          or isinstance(curve, Arc)):
        t_upper = 1
        t_lower = 0
        iteration = 0
...
class Line(object):
    ...
    def intersect(self, other_seg, tol=None):
        ...
        if isinstance(other_seg, Line):
            ...
            denom = ((a[1] - a[0]) * (d[0] - d[1]) - (b[1] - b[0]) *
                     (c[0] - c[1]))
            if np.isclose(denom, 0):
                return []
            t1 = (c[0] * (b[0] - d[1]) - c[1] * (b[0] - d[0]) - a[0] *
                  (d[0] - d[1])) / denom
            t2 = -(a[1] * (b[0] - d[0]) - a[0] * (b[1] - d[0]) - c[0] *
                   (b[0] - b[1])) / denom
...
class QuadraticBezier(object):
    ...
    def is_smooth_from(self, previous, warning_on=True):
        ...
        if isinstance(previous, QuadraticBezier):
            return (self.start == previous.end
                    and (self.control - self.start) == (previous.end -
                                                        previous.control))
        else:
            return self.control == self.start

    def joins_smoothly_with(self,
                            previous,
                            wrt_parameterization=False,
                            error=0):
        """Checks if this segment joins smoothly with previous segment.  By
        default, this only checks that this segment starts moving (at t=0) in
        the same direction (and from the same positive) as previous stopped
    ...
    def point(self, t):
        """returns the coordinates of the Bezier curve evaluated at t."""
        return (1 - t)**2 * self.start + 2 * (
            1 - t) * t * self.control + t**2 * self.end
    ...
    def reversed(self):
        ...
        if self._length_info['length']:
            ...
            new_quad._length_info['bpoints'] = (self.end, self.control,
                                                self.start)
        return new_quad
    ...
    def intersect(self, other_seg, tol=1e-12):
        ...
        if isinstance(other_seg, Line):
            ...
            return bezier_intersections(self,
                                        other_seg,
...
class CubicBezier(object):
    ...
    def is_smooth_from(self, previous, warning_on=True):
        ...
        if isinstance(previous, CubicBezier):
            return (self.start == previous.end
                    and (self.control1 - self.start) == (previous.end -
                                                         previous.control2))
        else:
    ...
    def reversed(self):
        ...
        if self._length_info['length']:
            ...
            new_cub._length_info['bpoints'] = (self.end, self.control2,
    ...
    def intersect(self, other_seg, tol=1e-12):
        ...
        if isinstance(other_seg, Line):
            return bezier_by_line_intersections(self, other_seg)
              ...
              or isinstance(other_seg, CubicBezier)):
            assert self != other_seg
            longer_length = max(self.length(), other_seg.length())
            return bezier_intersections(self,
                                        other_seg,
...
class Arc(object):
    ...
    def __repr__(self):
        params = (self.start, self.radius, self.rotation, self.large_arc,
                  self.sweep, self.end)
    ...
    def _parameterize(self):
        ...
        # the unit circle
        u1 = (x1p - cp.real) / rx + 1j * (y1p -
                                          cp.imag) / ry  # transformed start
        u2 = (-x1p - cp.real) / rx + 1j * (-y1p -
                                           cp.imag) / ry  # transformed end
    ...
    def point_to_t(self, point):
        ...
        if self.rotation != 0.0:
            raise ValueError(
                "Arc.point_to_t() only works on non-rotated Arcs.")
        ...
        if (distance_from_center < min_radius) and not np.isclose(
                distance_from_center, min_radius):
            return None
        if (distance_from_center > max_radius) and not np.isclose(
                distance_from_center, max_radius):
            return None
    ...
    def u1transform(self, z):
        ...
        zeta = (1 / self.rot_matrix) * (z - self.center
                                        )  # same as centeriso(z)
    ...
    def length(self,
        ...
        if _quad_available:
            return quad(lambda tau: abs(self.derivative(tau)),
                        t0,
                        t1,
                        epsabs=error,
                        limit=1000)[0]
    ...
    def derivative(self, t, n=1):
        ...
        if n % 4 == 0 and n > 0:
            return rx * cos(phi) * cos(angle) - ry * sin(phi) * sin(
                angle) + 1j * (rx * sin(phi) * cos(angle) +
                               ry * cos(phi) * sin(angle))
        elif n % 4 == 1:
            return k * (
                -rx * cos(phi) * sin(angle) - ry * sin(phi) * cos(angle) + 1j *
                (-rx * sin(phi) * sin(angle) + ry * cos(phi) * cos(angle)))
        elif n % 4 == 2:
            return k * (
                -rx * cos(phi) * cos(angle) + ry * sin(phi) * sin(angle) + 1j *
    ...
    def intersect(self, other_seg, tol=1e-12):
        ...
        if (self.rotation == 0) and isinstance(other_seg, Line):
            ...
            l = Line(start=(other_seg.start - self.center),
                     end=(other_seg.end - self.center))
            ...
            longer_length = max(self.length(), other_seg.length())
            inters = bezier_intersections(self,
                                          other_seg,
                                          longer_length=longer_length,
                                          tol=tol,
                                          tol_deC=tol)
    ...
    def cropped(self, t0, t1):
        ...
        if abs(self.delta * (t1 - t0)) <= 180:
            ...
            new_large_arc = 1
        return Arc(self.point(t0),
                   radius=self.radius,
                   rotation=self.rotation,
                   large_arc=new_large_arc,
                   sweep=self.sweep,
                   end=self.point(t1),
...
def is_bezier_segment(x):
    return (isinstance(x, Line) or isinstance(x, QuadraticBezier)
            or isinstance(x, CubicBezier))
...
class Path(MutableSequence):
    ...
    def __repr__(self):
        return "Path({})".format(",\n     ".join(
            repr(x) for x in self._segments))
    ...
    def _calc_lengths(self, error=LENGTH_ERROR, min_depth=LENGTH_MIN_DEPTH):
        ...
        lengths = [
            each.length(error=error, min_depth=min_depth)
    ...
    def point(self, pos):
        ...
        for index, segment in enumerate(self._segments):
            ...
            if segment_end >= pos:
                ...
                segment_pos = (pos - segment_start) / (segment_end -
                                                       segment_start)
    ...
    def iscontinuous(self):
        ...
        parameterization."""
        return all(self[i].end == self[i + 1].start
    ...
    def curvature(self, T):
        ...
        if np.isclose(t, 0) and (seg_idx != 0 or self.end == self.start):
            previous_seg_in_path = self._segments[(seg_idx - 1) %
                                                  len(self._segments)]
            if not seg.joins_smoothly_with(previous_seg_in_path):
                return float('inf')
        elif np.isclose(t, 1) and (seg_idx != len(self) - 1
                                   or self.end == self.start):
            next_seg_in_path = self._segments[(seg_idx + 1) %
                                              len(self._segments)]
    ...
    def intersect(self, other_curve, justonemode=False, tol=1e-12):
        ...
        if intersection_list:
            ...
            intersection_list = [
                inter for ind, inter in enumerate(intersection_list)
                if ind not in indices2remove
            ]
    ...
    def bounding_box(self):
        ...
        (xmin, xmax, ymin, ymax)."""
        bbs = [seg.bounding_box() for seg in self._segments]
